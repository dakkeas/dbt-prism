...
-- VETTING VERSION: Uses acn_raw_claims source tables instead of raw_claims_2023_2025 / raw_claims_2022
-- Purpose: to check dataset integrity

WITH acn_raw_claims_2022 AS (
    SELECT * FROM {{ source('mxc_raw_claims','acn_claims_2022') }} 
),
acn_raw_claims_2023_2025 AS (
...
cool_off_period AS ( -- produces unique patients
    SELECT
        ...
        FROM  (
            ...
            FROM
                acn_raw_claims_2023_2025 rc2
            LEFT JOIN -- selects claims from 2023 - 2025 data
                acn_raw_claims_2022 rc1
            ON 
        ...
        INNER JOIN acn_raw_claims_2023_2025 rc -- joining with raw
...
aggregate_starting_claim AS (
    ...
    INNER JOIN acn_raw_claims_2023_2025 rc
