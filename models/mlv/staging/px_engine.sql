...
WITH patient_engine AS (
    ...
    -- OP LAB
        ...
        ROUND(SUM(subsequent_sum_of_util_cptcode)::NUMERIC, 2) AS overall_cptcode_util,

        SUM(CASE WHEN subsequent_loatype='OP LAB' THEN subsequent_count_of_cptcode ELSE 0 END) AS opl_cptcode_coc,
        ROUND(SUM(CASE WHEN subsequent_loatype='OP LAB' THEN subsequent_sum_of_util_cptcode ELSE 0 END)::NUMERIC, 2) AS opl_cptcode_util,

        SUM(CASE WHEN subsequent_loatype='INPATIENT' THEN subsequent_count_of_cptcode ELSE 0 END) AS inp_cptcode_coc,
        ROUND(SUM(CASE WHEN subsequent_loatype='INPATIENT' THEN subsequent_sum_of_util_cptcode ELSE 0 END)::NUMERIC, 2) AS inp_cptcode_util,

        SUM(CASE WHEN subsequent_loatype='EMERGENCY' THEN subsequent_count_of_cptcode ELSE 0 END) AS emg_cptcode_coc,
        ROUND(SUM(CASE WHEN subsequent_loatype='EMERGENCY' THEN subsequent_sum_of_util_cptcode ELSE 0 END)::NUMERIC, 2) AS emg_cptcode_util,

        -- RUV Code
        SUM(subsequent_count_of_ruvcode) AS overall_ruvcode_coc,
        ROUND(SUM(subsequent_sum_of_util_ruvcode)::NUMERIC, 2) AS overall_ruvcode_util,

        SUM(CASE WHEN subsequent_loatype='OP LAB' THEN subsequent_count_of_ruvcode ELSE 0 END) AS opl_ruvcode_coc,
        ROUND(SUM(CASE WHEN subsequent_loatype='OP LAB' THEN subsequent_sum_of_util_ruvcode ELSE 0 END)::NUMERIC, 2) AS opl_ruvcode_util,

        SUM(CASE WHEN subsequent_loatype='INPATIENT' THEN subsequent_count_of_ruvcode ELSE 0 END) AS inp_ruvcode_coc,
        ROUND(SUM(CASE WHEN subsequent_loatype='INPATIENT' THEN subsequent_sum_of_util_ruvcode ELSE 0 END)::NUMERIC, 2) AS inp_ruvcode_util,

        SUM(CASE WHEN subsequent_loatype='EMERGENCY' THEN subsequent_count_of_ruvcode ELSE 0 END) AS emg_ruvcode_coc,
        ROUND(SUM(CASE WHEN subsequent_loatype='EMERGENCY' THEN subsequent_sum_of_util_ruvcode ELSE 0 END)::NUMERIC, 2) AS emg_ruvcode_util
