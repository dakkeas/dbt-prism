...
WITH patient_engine AS (
    SELECT
        ...
        -- STRING_AGG(DISTINCT crf_cptcodes, ', ') AS unique_crf_cptcodes,
        STRING_AGG(crf_cptcodes, ', ') AS crf_cptcodes,
        SUM(pre_dialysis_cptcode_count) AS pre_dialysis_cptcode_count,
        SUM(on_dialysis_cptcode_count) AS on_dialysis_cptcode_count,
        -- CRF Category
        MIN(crf_category) AS crf_category
