...
WITH acn_raw_claims_2023_2025 AS (
    SELECT * FROM {{ source('mxc_raw_claims','acn_claims_2023') }}
    UNION ALL
    SELECT * FROM {{ source('mxc_raw_claims','acn_claims_2024') }}
    UNION ALL
    SELECT * FROM {{ source('mxc_raw_claims','acn_claims_2025') }}
),
first_claim_details AS (
    SELECT
        fc.maskedcardno,
        MIN(rc.membertypedesc) AS starting_membertypedesc,
        MIN(rc.corpname) AS corpname,
        MIN(rc.claimno) AS starting_claimno,
        MIN(rc.relationship) AS starting_relationship,
...
subsequent_details AS (
    SELECT
        ...
        MIN(rc.relationship) AS subsequent_relationship,
    ...
    FROM
        {{ref('acn_subsequent_claims')}} s
    INNER JOIN
        acn_raw_claims_2023_2025 rc ON s.subsequent_claimno = rc.claimno
    LEFT JOIN
        {{ref('acn_prim_physician')}} pd ON s.subsequent_claimno = pd.subsequent_claimno
    GROUP BY
        s.maskedcardno, 
