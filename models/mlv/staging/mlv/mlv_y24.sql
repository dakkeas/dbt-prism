...
-- 2024 Cohort Version of MLV staging model
...
first_claim_details AS (
    SELECT
        ...
        MIN(rc.corpname) AS starting_corpname,
        MIN(rc.membertypedesc) AS starting_membertypedesc,
        MIN(rc.relationship) AS starting_member_relationship,
        ...
        SUM(
            CASE
                WHEN rc.coverageitemdesc IN ('SURGEON',
                'CONSULT/ATTENDING PHYSICIAN', 
                'DOCTOR SERVICES',
                'ANESTHESIOLOGIST'
                ) THEN ABS(rc.approved)
                ELSE 0 END
...
subsequent_details AS (
    SELECT
        ...
        MIN(rc.corpname) AS subsequent_corpname,
        MIN(rc.membertypedesc) AS subsequent_membertypedesc,
        MIN(rc.relationship) AS subsequent_member_relationship,
        ...
        SUM(
            CASE
                ...
                ELSE 0 END
        ) AS subsequent_professional_fees,
    ...
    FROM
        {{ ref('subsequent_claims_y24') }} s
    INNER JOIN
        raw_claims_window rc ON s.subsequent_claimno = rc.claimno
    LEFT JOIN
        {{ ref('prim_physician_y24') }} pd ON s.subsequent_claimno = pd.subsequent_claimno
    GROUP BY
        s.maskedcardno, 
...
merged_table AS (
    SELECT
        ...
        fc.starting_claimno,
        fc.starting_corpname,
        fc.starting_membertypedesc,
        fc.starting_member_relationship,
        ...
        fc.starting_professional_fees,
        COALESCE(fc.total_availment_cost, 0) AS total_pcc_availment_cost,
        COALESCE(fc.total_availment_count, 0) AS total_pcc_availment_count,
        fc.earliest_availment_date AS earliest_pcc_availment_date,
        fc.latest_availment_date AS latest_pcc_availment_date,

        s.claim_sequence,
        s.subsequent_claimno,
        s.subsequent_corpname,
        s.subsequent_membertypedesc,
        s.subsequent_member_relationship,
