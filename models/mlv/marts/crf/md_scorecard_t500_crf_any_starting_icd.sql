...
patient_engine AS (
    SELECT
        mlv.maskedcardno,
        MIN(mlv.computed_primaryicdgroup) AS computed_primaryicdgroup,
    ...
    FROM {{ ref('mlv_crf_any_starting_icd') }} mlv
...
provider_engine AS (
    ...
    FROM px_engine
    GROUP BY starting_providername, computed_primaryicdgroup
),
...
top_providers AS (
    ...
    WHERE computed_primaryicdgroup = 'CHRONIC RENAL FAILURE'
...
physician_engine AS (
    SELECT
        ...
        starting_providername AS providername,
        MIN(computed_primaryicdgroup) AS computed_primaryicdgroup,
    ...
    LEFT JOIN (
        ...
        ON TRIM(UPPER(pe.starting_physiciancode)) =
            ...
            {% endif %}
    WHERE pe.computed_primaryicdgroup = 'CHRONIC RENAL FAILURE'
...
WHERE providername IN (SELECT providername FROM top_providers)
    AND total_unique_patient_cnt > 3
