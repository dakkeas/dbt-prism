...
t10_list AS (
    -- grabs top 10 providers in total util for CRF cohort
    SELECT
...
FROM {{ ref('mlv_crf_any_starting_icd') }} m
LEFT JOIN crf_summary s
...
WHERE
    ...
    and
    m.computed_primaryicdgroup = 'CHRONIC RENAL FAILURE'
GROUP BY 1
