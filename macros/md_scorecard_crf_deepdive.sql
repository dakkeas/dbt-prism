...
## Data Sources
- `px_engine_crf_deepdive`: Source for physician claim and utilization data
...
{% macro md_scorecard_crf_deepdive(primaryicdgroup_list, top_n_provider, top_n_physicians, more_than_n_patients) %}
...
WITH physician_engine AS (
    SELECT
        ...
        -- CRF CATEGORY
        COUNT(DISTINCT CASE WHEN crf_category = 'Non CRF Patient' THEN maskedcardno END) AS count_of_non_crf_patients,
        COUNT(DISTINCT CASE WHEN crf_category = 'On-Dialysis' THEN maskedcardno END) AS count_of_on_dialysis_patients,
        COUNT(DISTINCT CASE WHEN crf_category = 'Pre-Dialysis' THEN maskedcardno END) AS count_of_pre_dialysis_patients,
        COUNT(DISTINCT CASE WHEN crf_category = 'CRF Only' THEN maskedcardno END) AS count_of_crf_only_patients
        

    FROM {{ ref('px_engine_crf_deepdive') }} pe
...
SELECT
    ...
    count_of_non_crf_patients,
    count_of_on_dialysis_patients,
    count_of_pre_dialysis_patients,
    count_of_crf_only_patients
